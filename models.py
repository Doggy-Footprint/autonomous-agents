# models.py
# 버그 리포팅 및 수정 시나리오를 위한 기능 추가

import datetime
import re

class World:
    """시뮬레이션 세계를 정의하고 관리합니다. 프로젝트 상태 보드가 추가되었습니다."""
    ...
    def execute_action(self, agent, action_string):
        ...
        if move_match:
            ...
            observation = f"{agent.name}이(가) 말합니다: \"{say_match.group(1).strip()}\""
        elif do_match:
            ...
            if feature in self.project_status:
                ...
                observation = f"{agent.name}이(가) 존재하지 않는 기능 '{feature}'의 버그를 보고하려 했습니다."
        elif fix_match:
            ...
            if feature in self.project_status:
                ...
                observation = f"{agent.name}이(가) 존재하지 않는 기능 '{feature}'의 버그를 수정하려 했습니다."
        else:
    ...
    def broadcast_observation(self, acting_agent, observation):
        for agent in self.agents.values():
            agent.add_memory(observation) # 이제 모든 에이전트가 모든 사건을 알게 됨

# Agent 클래스는 이전 3단계 버전과 동일합니다.
# (실제 코드에서는 Agent 클래스 코드가 이 부분에 있어야 합니다)
class Agent:
